...
<c:if test="${not empty AImessage}">
	<script
		src="${pageContext.request.contextPath}/static/js/ai_studio/novel_tutorial.js"></script>
	<link rel="stylesheet"
		href="${pageContext.request.contextPath}/static/css/ai_studio/novel_tutorial.css">
</c:if>
</head>
<body>
				...
				<form method="POST"
					...
					<div class="form-group">
						<label for="title">소설 제목</label> <input type="text"
							...
							placeholder="소설 제목을 입력하세요" required>
					</div>
					...
					<!-- 이미지 선택 -->
					<div class="form-group">
						<label for="imageSelect">소설 표지 이미지 선택</label> <select
							id="imageSelect" class="form-control" name="illust"
							onchange="displaySelectedImage()">
							<option value="">이미지를 선택하세요</option>
							<c:forEach var="image" items="${imageList}">
								<option value="${image.creationId}"
									data-image-url="${image.imageUrl}">${image.title != null ? image.title : image.imageUrl}
								</option>
							</c:forEach>
						</select>
					</div>
					...
					<!-- 선택한 이미지 미리보기 -->
					<div class="mt-3">
						<img id="selectedImagePreview" src="" alt="선택된 이미지가 여기에 표시됩니다"
							style="max-width: 100%; height: auto; display: none;" />
					</div>

					<!-- 선택한 이미지 파일명 표시 -->
					<p>
						선택한 파일명: <span id="selectedImageFileName">없음</span>
					</p>
					...
					<div class="form-group mt-4">
						...
						<textarea class="form-control" id="intro" name="intro" rows="6"
							placeholder="소설 줄거리를 입력하세요" required></textarea>
					</div>
