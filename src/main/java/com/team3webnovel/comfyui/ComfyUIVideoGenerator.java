...
//    }
	...
	// 프롬프트 전송 메서드
	public CompletableFuture<resultVo> vidqueuePrompt(String samplerIndex, int steps, int width, int height,
			int cfgScale, // CFG Scale만 double 타입으로 유지
			int seed, int clientId, String filenam) throws Exception {
				  ...
				  "11": {
				    ...
				    "_meta": {
				      "title": "SaveAnimatedPNG"
				        ...
				        """.formatted(width, height, seed, steps, 1.5, samplerIndex, filenam, clientId);
		...
		try {
			...
			if ("executed".equals(messageType)) {
				...
				for (int i = 0; i < images.length(); i++) {
					...
					// 이미지 URL 생성
					String generatedImageUrl = "http://" + SERVER_ADDRESS + "/view?filename=" + filename + "&subfolder="
							+ subfolder + "&type=output&nocache=" + System.currentTimeMillis();
	...
	public void onError(Session session, Throwable throwable) {
		...
		if (resultVoFuture != null) {
			resultVoFuture.completeExceptionally(throwable);
		} else {
			System.err.println("Error: resultVoFuture is null. Cannot completeExceptionally.");
		}
	}
}